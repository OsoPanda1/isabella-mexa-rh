/**
 * ================================================================
 * ISABELLA VILLASEÑOR AI — BETA / CAPABILITY REGISTRY
...
export class CapabilityRegistry {
  ...
  select(params: {
    ...
    const candidates = Array.from(this.capabilities.values()).filter((cap) => {
      ...
      if (params.constraints?.maxCostUsd !== undefined && cap.riskLevel === "critical") {
        return false;
      }
    ...
    const sorted = candidates.sort((a, b) => {
      const riskOrder: Record<ToolCapability["riskLevel"], number> = {
        low: 0,
        medium: 1,
        high: 2,
        critical: 3,
